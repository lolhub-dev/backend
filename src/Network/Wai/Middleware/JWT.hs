...
jwt sec ignored_routes app req sendRsp =
  let jwtAuth v = do
        maybe
          (sendRsp reject)
          (ifExpired . claims)
          (decodeAndVerifySignature (hmacSecret sec) v)
      decodeSubject :: StringOrURI -> Request -> Request
      decodeSubject sub req = do
        req { requestHeaders =
                ("USER", encodeUtf8 $ stringOrURIToText sub):requestHeaders req
            }
  ...
  in if length
       (filter
          (\x -> BS.isPrefixOf (encodeUtf8 x) (rawPathInfo req))
          ignored_routes)
       > 0
     then app req sendRsp
       ...
       (case lookup "Authorization" headers of
          ...
          Just authHead -> case BS.break isSpace authHead of
            (strategy, token)
              | BS.map toLower strategy
                == "bearer" -> return $ BS.dropWhile isSpace token
              | otherwise -> mempty)
  where
